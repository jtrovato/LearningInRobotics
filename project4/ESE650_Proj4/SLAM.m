...
for i=20:20:numel(ts)
    ...
    mask = (lidar{idx}.scan < 50) & (lidar{idx}.scan >0.5); %& lidar{idx}.scan > 0.5);
    ...
    %rotate and shift prior mask according to offsets
    pm_r = imrotate(pm, (previous_pose(3)+ds(3))*(180/pi), 'crop');
